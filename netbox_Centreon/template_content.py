...
class NagiosStatus(PluginTemplateExtension):
    def __init__(self, context):
        ...
        self.centreon_base_url = self.get_centreon_base_url()
    ...
    def get_centreon_base_url(self):
        """Uses settings and potential overrides to determine the Nagios url."""
        for regex, centreon_base_url in self.settings["centreon_base_url_overrides"]:
            if re.search(regex, self.hostname):
                return centreon_base_url
        return self.settings["centreon_base_url"]

    def buttons(self):
        """Adds an extra button at the top of the bage."""
        return self.render(
            "device_centreon_buttons.html",
            extra_context={"centreon_base_url": self.centreon_base_url},
        )

    def right_page(self):
        """Adds a status table to the page."""
        extra_context = {
            "centreon_base_url": self.centreon_base_url,
        }
        try:
